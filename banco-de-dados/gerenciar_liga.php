<?php
 ...
 $action = $_POST['action'] ?? '';
 if (empty($action)) {
   header('Location: ../jogo/ligas.php?error=invalid_action');
   exit();
 }
 ...
 switch ($action) {  // para criar uma liga
   ...
   if (empty($league_name) || empty($league_keyword)) {
     header('Location: ../jogo/ligas.php?error=empty_fields');
     exit();
   }

   // inserção da nova liga + seu ID
   $stmt_create = $pdo->prepare("INSERT INTO table_leagues (nome, palavra_chave, id_criador_liga) VALUES (?, ?, ?)");
   $stmt_create->execute([$league_name, $league_keyword, $id_usuario]);
   
   // retorna o ID da liga criada
   $new_league_id = $pdo->lastInsertId();

   // adc o criador da liga como primeiro membro dela
   $stmt_join = $pdo->prepare("INSERT INTO table_league_members (id_usuario, id_liga) VALUES (?, ?)");
   $stmt_join->execute([$id_usuario, $new_league_id]);

   header('Location: ../jogo/ligas.php?success=league_created');
   break;
   ...
   if ($league && $league['palavra_chave'] === $keyword_attempt) {
     try {
       ...
       header('Location: ../jogo/ligas.php?success=joined_league');
     } catch (PDOException $e) {  // erro caso o usuário já seja membro da liga
       if ($e->getCode() == 23000) {
         header('Location: ../jogo/ligas.php?error=already_member');
       } else {
         header('Location: ../jogo/ligas.php?error=db_error');
       }
     }
   } else {
     header('Location: ../jogo/ligas.php?error=wrong_keyword');
   }
   break;
   ...
   $stmt = $pdo->prepare("DELETE FROM table_league_members WHERE id_usuario = ? AND id_liga = ?");
   $stmt->execute([$id_usuario, $league_id]);

   header('Location: ../jogo/ligas.php?success=left_league');
   break;
   ...
   $stmt_check = $pdo->prepare("SELECT id_criador_liga FROM table_leagues WHERE id_liga = ?");
   $stmt_check->execute([$league_id]);
   $league = $stmt_check->fetch();

   if ($league && $league['id_criador_liga'] == $id_usuario) {
     $stmt_delete = $pdo->prepare("DELETE FROM table_leagues WHERE id_liga = ?");
     $stmt_delete->execute([$league_id]);

     header('Location: ../jogo/ligas.php?success=league_deleted');
   } else {
     header('Location: ../jogo/ligas.php?error=unauthorized');
   }
   break;

   default:
   header('Location: ../jogo/ligas.php?error=unknown_action');
   break;
 }
